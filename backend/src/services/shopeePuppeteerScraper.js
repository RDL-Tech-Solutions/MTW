...
class ShopeePuppeteerScraper {
    ...
    async scrapeProduct(url) {
        ...
        try {
            console.log('🤖 Iniciando Puppeteer para Shopee...');
            ...
            const data = await page.evaluate(() => {
                ...
                // Nome do produto - Seletores atualizados 2024/2025
                const nameSelectors = [
                    '.attM6y span', // Título comum
                    '.Vnont8', // Título obfuscated
                    '.product-briefing', // Container do título
                    '.pdp-product-title',
                ...
                // Descrição
                const descriptionSelector = '.product-detail, [data-testid="product-detail"], .page-product__detail';
                const descriptionEl = document.querySelector(descriptionSelector);
                const description = descriptionEl ? cleanText(descriptionEl.innerText) : '';
                ...
                for (const selector of descSelectors) {
                    const el = document.querySelector(selector);
                    if (el) {
                        result.description = el.content || el.textContent.trim();
